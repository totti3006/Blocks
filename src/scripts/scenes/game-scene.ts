...
export class GameScene extends Phaser.Scene {
  ...
  init(): void {
    ...
    for (let y = 0; y < this.currentLevelHeight; y++) {
      for (let x = 0; x < this.currentLevelWidth; x++) {
        ...
        this.currentLevelArray.push(
          new Block({
            scene: this,
            x: x * CONST.tileSize + CONST.tileSize * 0.5,
            y: y * CONST.tileSize + CONST.tileSize * 0.5,
            texture: 'block',
            type: blockType
          })
        )
    ...
    this.cursor = new Cursor({
      scene: this,
      x: CONST.levels[CONST.currentLevel].cursorStart[0] * CONST.tileSize + CONST.tileSize * 0.5,
      y: CONST.levels[CONST.currentLevel].cursorStart[1] * CONST.tileSize + CONST.tileSize * 0.5,
      texture: 'cursor',
  ...
  private handleInput(): void {
    ...
    if (dx !== 0 || dy !== 0) {
      ...
      if (this.getBlockType(newX, newY) !== 1) {
        ...
        // this.getBlock(newX, newY).playScaleAnimation()
        this.getBlock(newX, newY).playJumpAnimation()
  ...
  swapTwoBlocks(blockId0: number, blockId1: number): void {
    ...
    this.currentLevelArray[blockId0].resetAnimation()
    this.currentLevelArray[blockId1].resetAnimation()
  }
  ...
  public isSameTypeAroundMe(x: number, y: number): boolean {
    ...
    if (me === leftOfMe || me === rightOfMe || me === topOfMe || me === bottomOfMe) {
      return true
    }
